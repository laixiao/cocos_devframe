...
export class EventManager {
    ...
    on(eventName: string, callBack: EventManagerCallFunc, target?: any): boolean {
        if (!eventName) {
    ...
    setEvent(eventName: string, callBack: EventManagerCallFunc, target?: any): boolean {
        if (!eventName) {
            warn("eventName is empty" + eventName);
            return false;
        }
    ...
    off(eventName: string, callBack: EventManagerCallFunc, target?: any) {
        if (null != this._eventListeners[eventName]) {
            let index = this.getEventListenersIndex(eventName, callBack, target);
            if (-1 != index) {
    ...
    emit(eventName: string, eventData?: any) {
        // console.log(`==================== emit ${eventName} begin | ${JSON.stringify(eventData)}`);
        if (null != this._eventListeners[eventName]) {
